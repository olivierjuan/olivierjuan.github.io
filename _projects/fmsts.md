...
variable to branch on at each node. The quality of these _branching strategies_ has an
...
the _same_ MILP problem with fluctuating data drawn from an unknown distribution $$\mathcal{D}$$.
...
same next state), exact Q-values become directly _observable_ after tree expansion — a key
...
- **Principled** — Proposition 2 (proved in the paper) shows that under Depth-First Search,
  minimising every local subtree size is _equivalent_ to minimising the global tree size.
...
**Proposition 1** (negative result): Minimising local subtree sizes is _not_ globally optimal
